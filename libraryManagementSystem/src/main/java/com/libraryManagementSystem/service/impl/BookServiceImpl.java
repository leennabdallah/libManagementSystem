...
public class BookServiceImpl implements BookService {
   ...
   /* if (book==null || book.getIsbn()==null || book.getIsbn().isEmpty()){
        throw new IllegalArgumentException(" Book or book ISBN cannot be null or empty");
    }*/
        ...
        for (int i=0;i<listeDeAuthors.size();i++) {
            ...
            if (authorRepository.getByName(nom)==null) {
                author.setName(nom);
                author= authorRepository.save(author);
            }
            else{
                author= authorRepository.getByName(nom);
